...
/**
 * Custom Imagine AbstractImage class that implements
 * different thumbails crop logic.
 *
 * To prevent editing Imagine lib vendor files,
 * the application is forced to use the current class when `Imagine\Image\AbstractImage` is called
 * (via the class map `@common/config/bootstrap.php`).
 *
    ...
    public function thumbnail(BoxInterface $size, $mode = ImageInterface::THUMBNAIL_INSET, $filter = ImageInterface::FILTER_UNDEFINED)
    {
        ...
        if ($mode === ImageInterface::THUMBNAIL_OUTBOUND) {
            ...
            $thumbnail->crop(new Point(
                ...
                // Vertical crop must start from top (https://github.com/ganigeorgiev/presentator/issues/24)
