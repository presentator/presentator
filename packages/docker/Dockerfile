...
RUN curl https://github.com/presentator/presentator-starter/archive/v2.7.0.tar.gz -L -o presentator.tar.gz \
    && tar -xvf presentator.tar.gz --strip 1 \
    && rm presentator.tar.gz

# require specific app dependency version
RUN composer require presentator/api:2.7.0 presentator/spa:2.7.0 -d /var/www/html --no-update --no-suggest --no-scripts
