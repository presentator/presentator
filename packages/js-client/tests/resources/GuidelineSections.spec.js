...
const GuidelineSections = require('@/resources/GuidelineSections');

/**
 * GuidelineSections API resource tests.
 *
 * @author Gani Georgiev <gani.georgiev@gmail.com>
 */
describe('GuidelineSections', function () {
    var adapter  = new mockAdapter(axios);
    var resource = new GuidelineSections(axios.create());
    ...
    describe('getList()', function () {
        it('Should correctly set request data', function (done) {
            ...
            result.then(function (response) {
                assert.equal(response.config.url, '/guideline-sections');
    ...
    describe('getOne()', function () {
        it('Should correctly set request data', function (done) {
            ...
            assert.instanceOf(result, Promise);
            result.then(function (response) {
                assert.equal(response.config.url, '/guideline-sections/123');
                assert.equal(response.config.method, 'get');
