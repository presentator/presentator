    ...
    <div class="auth-container">
        ...
        <div class="container-wrapper container-wrapper-sm">
            ...
            <div class="panel auth-panel">
                <h3 class="panel-title">{{ $t('Account activation') }}</h3>

                <div class="panel-content">
                    <div v-if="isProcessing" class="alert alert-transp-primary txt-center">
                        <p>{{ $t('Activating your account') }} <span class="loader m-l-5"></span></p>
                    </div>

                    <template v-else>
                        <div v-if="!processSuccess" class="alert alert-transp-danger txt-center">
                            <p>
                                {{ $t('The provided activation token is invalid or expired.') }} <br>
...
export default {
    ...
    beforeMount() {
        this.$setDocumentTitle(() => this.$t('Account activation'));
    ...
    methods: {
        activate() {
            ...
            ApiClient.Users.activate(this.$route.params.activateToken).then((response) => {
