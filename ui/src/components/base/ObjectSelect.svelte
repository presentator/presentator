<script>
    ...
    function handleKeyOfSelectedChange(newKeyOfSelected) {
        newKeyOfSelected = utils.toArray(newKeyOfSelected, true);

        let newSelected = [];

        for (let v of newKeyOfSelected) {
            const item = utils.findByKey(items, selectionKey, v);
            if (item) {
    ...
    async function handleSelectedChange(newSelected) {
        let extractedKeys = utils.toArray(newSelected, true).map((item) => item[selectionKey]);
